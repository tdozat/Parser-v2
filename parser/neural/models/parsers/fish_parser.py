...
class FishParser(BaseParser):
  ...
  def __call__(self, vocabs, moving_params=None):
    ...
    with tf.variable_scope('Lambda'):
      ...
      arc_lambdas = self.bilinear(lambda_dep_mlp, lambda_head_mlp, 1)
    with tf.variable_scope('Arc'):
      ...
      arc_logits += (k_mat-1)*lambdas - tf.exp(lambdas) - tf.lgamma(k_mat)
    ...
    with tf.variable_scope('Rel'):
      ...
      select_rel_logits = tf.matlambdal(rel_logits, one_hot)
