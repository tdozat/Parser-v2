  ...
  def __call__(self, vocabs, moving_params=None):
    ...
    with tf.variable_scope('MLP'):
      dep_mlp, head_mlp = self.MLP(top_recur, self.arc_mlp_size + self.rel_mlp_size + 2*self.p_mlp_size,
                                   n_splits=2)
      arc_dep_mlp, rel_dep_mlp, p_dep_mlp, r_dep_mlp = tf.split(dep_mlp, [self.arc_mlp_size, self.rel_mlp_size, self.p_mlp_size, self.r_mlp_size], axis=2)
      arc_head_mlp, rel_head_mlp, mu_head_mlp = tf.split(head_mlp, [self.arc_mlp_size, self.rel_mlp_size, self.p_mlp_size, self.r_mlp_size], axis=2)
    
    with tf.variable_scope('p'):
      # (n x b x d) o (d x 1 x d) o (n x b x d).T -> (n x b x b)
      arc_ps = tf.exp(self.bilinear(mu_dep_mlp, mu_head_mlp, 1))
    with tf.variable_scope('r'):
      # (n x b x d) o (d x 1 x d) o (n x b x d).T -> (n x b x b)
      arc_rs = tf.nn.sigmoid(self.bilinear(mu_dep_mlp, mu_head_mlp, 1))
    with tf.variable_scope('Arc'):
      ...
      arc_logits += (tf.lgamma(k_mat + arc_rs-1) - tf.lgamma(k_mat) - tf.lgamma(arc_rs) +
                     arc_rs * tf.log(1-arc_ps) + (k_mat-1)*tf.log(arc_ps) )
      # (n x b x b) - ((b x b) + (b x b)) -> (n x b x b)
      arc_logits -= tf.log(
        tf.to_float(k_mat < i_mat) +
        tf.to_float(k_mat < n_mat) )
      # (n x b x b)
